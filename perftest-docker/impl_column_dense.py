...
def perftest_until_n(n: int, force=False, output_dir="timings"):
    with db.connect(DBNAME) as con:
        ...
        perftest.measure_performance(ColumnBasedIntermediate(), force=force)


def perftest_dense_constraints(n: int, dimensions=3):
    def generate_constraints(num_constraints: int):
        import random
        random.seed(123)
    ...
    with db.connect(DBNAME) as con:
        class ColumnBasedDenseConstraints(perftest.PerfTest):
            def name(self):
                return "column_based_dense_constraints"

            def setup_run(self, num_constraints):
                #constraints = generate_constraints(num_constraints)
                #generate_db_for_constraints(con, constraints)
                pass

            def run(self, num_constraints):
                constraints = generate_constraints(num_constraints)
                create_cad(con, constraints)
                result = con.execute("SELECT * FROM Result WHERE truth_value")
        ...
        perftest.measure_performance(ColumnBasedDenseConstraints(), force=True)

print("Perftesting!")
perftest_dense_constraints(20)
